...
let minSugar=document.getElementById('minSugar').value;
let maxSugar=document.getElementById('maxSugar').value;
...
const url=`${apiUrl}?apiKey=${apiKey}&${filtersString ? `${filtersString}` :''}`;
...
fetch(url).then((res)=>{
  ...
  resultHeading.innerHTML = `<p>At least one filter should be applied!<p>`;
  }
...
single_mealEl.innerHTML = `
  <div class="single-meal">
    ...
    <div class="single-meal-info">
      ...
      ${meal.vegetarian ? `<p>Vegetarian</p>` : '<p>Non Vegetarian</p>'}
      ${meal.readyInMinutes? `<p>Preparation Time:${meal.readyInMinutes}mins</p>` : ''}
    </div>
    <div class="main">
